...
def plot_country(temp, country = 'Germany', year = 1960):
    ...
    temp_filt_groupsum_all = temp.groupby(['Country', 'Year']).agg({'Value': np.median}).reset_index()
    
    # group sum for specified country
    temp_filt_groupsum = temp_filt.groupby('Year').agg({'Value': np.median}).reset_index()
